...
--  ███████║██║  ██║╚██████╗██║  ██║███████╗██████╔╝    ██║  ██║██║  ██║██║  ██║
--  ╚══════╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═════╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝
--                             OPEN SOURCE SCRIPT by iSacredRivals
...
function LoadConfig()
    pcall(function()
        if isfile and readfile and isfile("SacredAHK_Config.json") then
            ...
            if conf.MacroBeta ~= nil then MacroEnabled = conf.MacroBeta end -- Do not auto-open macroGui
...
function GetCamlockTarget()
    ...
    if CamlockPlayerEnabled then
        if SelectedSoruTarget and SelectedSoruTarget ~= "Nearest" and SelectedSoruTarget ~= "" then
            local tPlayer = Players:FindFirstChild(SelectedSoruTarget)
            if tPlayer and tPlayer.Character then
                ...
                if thrp and thum and thum.Health > 0 then return thrp end
            end
        end
...
local currentThemeColor = THEMES["Neon Purple"] -- Color por defecto Morado
local COLORS = {
    Background = Color3.fromRGB(0, 0, 0), -- Pure Transparent
    PanelBG = Color3.fromRGB(0, 0, 0), -- Pure Transparent
    TextWhite = Color3.fromRGB(255, 255, 255),
    TextGray = Color3.fromRGB(200, 200, 210),
    ToggleOff = Color3.fromRGB(0, 0, 0), -- Pure Transparent
}
...
-- Asegurar que la interfaz del juego (PlayerGui.Main) esté siempre visible
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            if player and player:FindFirstChild("PlayerGui") then
                ...
                if mainGui then
                    if not mainGui.Enabled then mainGui.Enabled = true end
                    local hotbar = mainGui:FindFirstChild("Hotbar")
                    if hotbar and not hotbar.Visible then hotbar.Visible = true end
                end
            end
        end)
    end
...
-- Asegurar que el HUD de Blox Fruits (PlayerGui.Main) NUNCA se oculte en PC
task.spawn(function()
...
GuiStore.camlockWidgetGui.Name = "SacredUI_CamlockWidget"
...
GuiStore.aimbotWidgetGui.Name = "SacredUI_AimbotWidget"
...
function makeFloatingWidget(parent, pos, title)
    ...
    btn.Position = pos
    btn.BackgroundColor3 = Color3.fromRGB(15, 15, 22) -- Fondo Negro/Gris Nítido
    btn.BackgroundTransparency = 0.25 -- Opaco para máxima legibilidad
...
function createTopControl(text, xOff, color, cb)
    ...
    btn.BackgroundTransparency = 1 -- 100% Transparente sin fondo gris feo
    btn.Text = text
...
mainTitle.Text = "SACRED AHK [MOBILE]"
...
-- Smooth custom drag handling for mainFrame (No legacy Draggable mouse lock on Alt-Tab)
...
RightPanel.BackgroundTransparency = 0.65 -- Cristal oscuro sutil que permite ver el wallpaper y hace 100% legibles los nombres de la lista!
...
TRANSLATIONS = {
    ...
    ES = {
        ...
        ["Sacred VFX & Portal"] = "Sacred VFX y Portal",
...
function createModuleCard(name, height, targetPage)
    ...
    card.BackgroundTransparency = 0.65 -- Cristal oscuro sutil que permite ver el wallpaper y hace 100% legibles las letras sobre cualquier fondo!
...
-- Botón Selector de Widget de Aimbot (Estilo Soru / Flashstep)
...
addToggleElement(c1, "Aimbot M1 (Dragon Gun) ⚠️ BAN RISK", _G.G_DragonGunM1, 68, function(v) 
    _G.G_DragonGunM1 = v
    UpdateDragonButton() 
end, "DragonM1")
...
function showMacroConfigUI()
    ...
    macroGui.Name = "Sacred_Macro_Config_UI"
...
function showFloatingComboTrigger(show)
    ...
    floatingTriggerGui.Name = "Sacred_Macro_Floating_Combo"
...
triggerSelectBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro negro!
...
weaponSelectBtn.BackgroundTransparency = 1 -- 100% Transparente
...
keySelectBtn.BackgroundTransparency = 1 -- 100% Transparente
...
function refreshPlayerListUI()
    ...
    nearestBtn.BackgroundTransparency = 1 -- 100% Transparente
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= player then
            ...
            pBtn.BackgroundTransparency = 1 -- 100% Transparente
            pBtn.Text = "👤 " .. p.Name
    ...
    if not ListScroll:FindFirstChild("RefreshBtn") then
        ...
        refreshBtn.BackgroundTransparency = 1 -- 100% Transparente
...
fpsOverlayGui.Name = "SacredUI_FPSOverlay"
...
local bgOptions = {
    ...
    { name = "Anime 9 (130251989349036)", id = "rbxassetid://130251989349036" },
    { name = "Anime 10 (127470325293722)", id = "rbxassetid://127470325293722" },
    { name = "Anime 11 (83824690471230)", id = "rbxassetid://83824690471230" },
    { name = "Anime 12 (104611310855431)", id = "rbxassetid://104611310855431" },
    { name = "Anime 13 (123658374940434)", id = "rbxassetid://123658374940434" },
    { name = "None", id = "none" }
}
...
bgSelectorBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro negro/gris!
...
for idx, tData in ipairs(themeBtnList) do
    ...
    tBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadros negros/grises!
...
for idx, item in ipairs(skyBtnGrid) do
    ...
    sBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro de color atras!
    sBtn.Text = item.name
...
for _, sInfo in ipairs(songsData) do
    ...
    local songToggle = addToggleElement(card, sInfo.name, false, 24, function(v)
        if v then
            ...
            snd.Name = "SacredSong_" .. sInfo.name
            ...
            if currentPlayingSound and currentPlayingSound.Name == "SacredSong_" .. sInfo.name then
                stopCurrentSong()
            end
        end
...
-- Sacred VFX & Change Skills Colors Engine (New Portal + Ice V2)
local PortalVFX_ON = false
local TRANSFORM_ON = false
local INCLUDE_GENERIC = false -- Change skills colors
...
local CUSTOM_GRADIENTS = {
    {name = "Ice", primary = Color3.fromRGB(0, 200, 255), seq = ColorSequence.new(Color3.fromRGB(0, 200, 255), Color3.fromRGB(255, 255, 255)), asset = "rbxassetid://82169952275337"},
    {name = "Pink", primary = Color3.fromRGB(255, 0, 150), seq = ColorSequence.new(Color3.fromRGB(255, 0, 150), Color3.fromRGB(200, 0, 255)), asset = "rbxassetid://82169952275337"},
    {name = "Lava", primary = Color3.fromRGB(255, 60, 0), seq = ColorSequence.new(Color3.fromRGB(255, 60, 0), Color3.fromRGB(255, 200, 0)), asset = "rbxassetid://82169952275337"},
    {name = "Toxic", primary = Color3.fromRGB(50, 255, 50), seq = ColorSequence.new(Color3.fromRGB(0, 255, 100), Color3.fromRGB(0, 255, 255)), asset = "rbxassetid://82169952275337"},
    {name = "Sunset", primary = Color3.fromRGB(255, 150, 0), seq = ColorSequence.new(Color3.fromRGB(255, 150, 0), Color3.fromRGB(255, 50, 50)), asset = "rbxassetid://82169952275337"}
}
...
local function updateInventoryIcon()
    pcall(function()
        ...
        for idx, slot in ipairs(frames) do
            ...
            local existing = border:FindFirstChild("SacredPortalIcon")

            local isPortalSlot = false
            if PortalVFX_ON and CURRENT_ASSET then
                local slotName = tostring(slot.Name):lower()
                local toolInside = slot:FindFirstChildOfClass("Tool") or slot:FindFirstChild("ToolName")
                local toolName = toolInside and tostring(toolInside.Value or toolInside.Name):lower() or ""

                if slotName == "2" or slot.LayoutOrder == 2 or (toolName ~= "" and toolName:find("portal")) then
                    isPortalSlot = true
                end
            end
            ...
            if isPortalSlot then
                if not existing then
                    ...
                    existing.Name = "SacredPortalIcon"
...
langBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro negro/gris!
...
copyBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro negro/gris!
...
copyTikTokBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro negro/gris!
...
saveBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro negro/gris!
...
resetBtn.BackgroundTransparency = 1 -- 100% Transparente sin recuadro negro/gris!
...
resetBtn.MouseButton1Click:Connect(function() 
    pcall(function() 
        if isfile and isfile("SacredAHK_Config.json") then delfile("SacredAHK_Config.json") end 
        if isfile and isfile("SacredAHK_Bounty.json") then delfile("SacredAHK_Bounty.json") end
    end)
...
print("✅ SACRED AHK [MOBILE] v13 LOADED - ALL TOGGLES AND CONFIGS 100% PERSISTENT")
...
task.spawn(function()
    pcall(function()
        local data = {
            content = "🚀 **SacredScripts**\n" ..
                      "👤 **Usuario:** " .. (player and player.Name or "Unknown") .. "\n" ..
                      "📊 **Ejecuciones:** " .. tostring(getgenv().execCount)
        }
        local req = request or http_request or (syn and syn.request) or (http and http.request)
        if req then
            req({
                Url = "https://discord.com/api/webhooks/1536957063400390719/65eSAT1x_7T_K9Pvkkhuqj-7ojmEWyg2et8HSJ6PayOwQfx77KI3vWqRXG1TgPR9aRTI",
                Method = "POST",
